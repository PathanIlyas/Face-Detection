...
class ReIDModel:
    ...
    def extract_features_batched(self, image_crops_bgr: List[np.ndarray]) -> np.ndarray:
        ...
        # Chunk large batches to stay within the engine's optimization profile max (8).
        MAX_BATCH_SIZE = 8
        n = input_torch_tensor.shape[0]
        all_features = []

        try:
            for start in range(0, n, MAX_BATCH_SIZE):
                chunk = input_torch_tensor[start : start + MAX_BATCH_SIZE]
                outputs_dict = self.trt_engine.infer({self.input_name: chunk})
                all_features.append(outputs_dict[self.output_name].detach().cpu().numpy())
        except KeyError as e:
            print(f"Error: Output tensor name '{e}' not found in ReID TRT engine outputs.")
            return np.empty((0, self.feature_dim), dtype=np.float32)
